...
namespace Microsoft.Health.CosmosDb.Features.Storage.Versioning
{
    public abstract class CollectionUpgradeManager : IUpgradeManager
    {
        ...
        public async Task SetupCollectionAsync(Container documentClient)
        {
            EnsureArg.IsNotNull(documentClient, nameof(documentClient));

            using (var cancellationTokenSource = new CancellationTokenSource(TimeSpan.FromMinutes(5)))
            {
                using (var distributedLock = _lockFactory.Create(documentClient, $"UpgradeLock-{CollectionSettingsVersion}"))
                {
                    ...
                    foreach (var updater in _collectionUpdater)
                    {
                        ...
                        await updater.ExecuteAsync(documentClient);
                    }
